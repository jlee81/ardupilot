...
void Mode::land_run_vertical_control(bool pause_descent)
{
    ...
    const float precland_acceptable_error = 45.0f;      // default : 15.0f;
    const float precland_min_descent_speed = 20.0f;         // default : 10.0f;
    ...
    if (!pause_descent) {
        ...
        if (doing_precision_landing && copter.rangefinder_alt_ok() && copter.rangefinder_state.alt_cm > 60.0f && copter.rangefinder_state.alt_cm < 200.0f) {        // default : 35.0f ~ 200.0f
            float max_descent_speed = abs(g.land_speed)*0.7f;       // default : *0.5f
