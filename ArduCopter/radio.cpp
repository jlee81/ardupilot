...
void Copter::radio_motor_switch()
{
    if ( RC_Channels::rc_channel(CH_6)->get_radio_in() > 1600 )
    {
        flag_disable_motor = true;
    }
    else 
    {
        flag_disable_motor = false;
    }

    if (flag_disable_motor_last != flag_disable_motor) 
    {
        if(flag_disable_motor)
        {   
//            motors->remove_motor(motor_num_disable);
            gcs().send_text(MAV_SEVERITY_CRITICAL, "Motor%d %d %% out", motor_num_disable+1, (uint8_t)(motor_ratio*100));
        }
        else 
        { 
            motor_num_disable++;
            if(motor_num_disable >= 4)
                motor_num_disable = 0;

/*            
            switch (motor_num_disable)
            {
                case 0u:
                    motors->add_motor(0U, 90, -1, 2);
                    break;
                case 1u:
                    motors->add_motor(1U, -90, 1, 5);
                    break;
                case 2u:
                    motors->add_motor(2U, -30, -1, 6);
                    break;
                case 3u:
                    motors->add_motor(3U, 150, 1, 3);
                    break;
                case 4u:
                    motors->add_motor(4U, 30, -1, 1);
                    break;
                case 5u:
                    motors->add_motor(5U, -150, 1, 4);
                    break;
                default:
                break;
            }          
*/            
            gcs().send_text(MAV_SEVERITY_CRITICAL, "Motor restored");
            
        }
        flag_disable_motor_last = flag_disable_motor;
    }

    motors->set_motor_switch(flag_disable_motor, motor_num_disable, motor_ratio);        

    uint16_t temp_RC_ch = RC_Channels::rc_channel(CH_9)->get_radio_in();

    if (temp_RC_ch >= 1880) {
        motor_ratio = 1.0;        
    } else if (temp_RC_ch >= 1820) {
        motor_ratio = 0.9;        
    } else if (temp_RC_ch >= 1740) {
        motor_ratio = 0.8;        
    } else if (temp_RC_ch >= 1660) {
        motor_ratio = 0.7;        
    } else if (temp_RC_ch >= 1580) {
        motor_ratio = 0.6;        
    } else if (temp_RC_ch >= 1490) {
        motor_ratio = 0.5;        
    } else if (temp_RC_ch >= 1420) {
        motor_ratio = 0.4;        
    } else if (temp_RC_ch >= 1340) {
        motor_ratio = 0.3;        
    } else if (temp_RC_ch >= 1250) {
        motor_ratio = 0.2;        
    } else if (temp_RC_ch >= 1180) {
        motor_ratio = 0.1;        
    } else {
        motor_ratio = 0;        
    }
