...
void Copter::ai_control2()
{
  ...
  _total_roll_output.value = attitude_control->get_output_roll();
  _total_pitch_output.value = attitude_control->get_output_pitch();
...
void Copter::control_out_tx()
{
  ...
  tx_pck[1] = 0x55;
  for(int i=0;i<4;i++) {
    tx_pck[2+i] = _total_roll_output.bytes[i];  
  }

  for(int i=0;i<4;i++) {
    tx_pck[6+i] = _total_pitch_output.bytes[i];  
  }
...
void Copter::control_in_rx()
{
  ...
  while(nbytes-- > 0)
  {
    uint8_t serialTemp = hal.uartE->read();
    ...
    switch( ucStatus )
    {
      ...
      case 8:
      case 9:
        RxDataBuf[ucCnt++] = serialTemp;
        RxChecksum += serialTemp;
        ucStatus++;
      break;

      case 10:
        if(RxChecksum == serialTemp)
        {
